...
object ApiTestUtils extends FunSuite {
  def assertArrayEquals[A](arr1: Array[A], arr2: Array[A]) = assert(arr1.deep == arr2.deep)
  ...
  def assertCompileFails(regex: String, flag: Int): Unit = {
    try {
      ...
      case e: IllegalArgumentException =>
        if (!("Flags UNIX_LINES and COMMENTS unsupported" == e.getMessage)) throw e
    }
  }
  ...
  /**
    ...
    */
  def assertArrayEquals(expected: Array[AnyRef], actual: Array[AnyRef]): Unit = {
    assert(expected.length == actual.length, "Arrays have unequal length, therefore can't be equal to " + "each other. Expected: " + util.Arrays.toString(expected) + " Actual: " + util.Arrays.toString(actual))
    var idx = 0
    while (idx < expected.length) {
      assert(expected(idx) == actual(idx), "Index: " + idx + " is unequal in the arrays")
      idx += 1
    }
  }
  ...
  def testMatches(regexp: String, `match`: String, nonMatch: String): Unit = {
    ...
    assert(java.util.regex.Pattern.matches(regexp, `match`), "JDK " + errorString + " doesn't match: " + `match`)
    assert(!java.util.regex.Pattern.matches(regexp, nonMatch), "JDK " + errorString + " matches: " + nonMatch)
    assert(Pattern.matches(regexp, `match`), errorString + " doesn't match: " + `match`)
    assert(!Pattern.matches(regexp, nonMatch), errorString + " matches: " + nonMatch)
  }

  // Test matches via a matcher.
  def testMatcherMatches(regexp: String, `match`: String, nonMatch: String): Unit = {
  ...
  def testMatcherMatches(regexp: String, `match`: String): Unit = {
    ...
    assert(p.matcher(`match`).matches, "JDK Pattern with regexp: " + regexp + " doesn't match: " + `match`)
    val pr = Pattern.compile(regexp)
    assert(pr.matcher(`match`).matches, "Pattern with regexp: " + regexp + " doesn't match: " + `match`)
  }

  def testMatcherNotMatches(regexp: String, nonMatch: String): Unit = {
    val p = java.util.regex.Pattern.compile(regexp)
    assert(!p.matcher(nonMatch).matches, "JDK Pattern with regexp: " + regexp + " matches: " + nonMatch)
    val pr = Pattern.compile(regexp)
    assert(!pr.matcher(nonMatch).matches, "Pattern with regexp: " + regexp + " matches: " + nonMatch)
  }
  ...
  /**
    ...
    *
    */
  def testMatchesRE2(regexp: String, flags: Int, `match`: String, nonMatch: String): Unit = {
  ...
  def testMatchesRE2(regexp: String, flags: Int, matches: Array[String], nonMatches: Array[String]): Unit = {
    val p = Pattern.compile(regexp, flags)
    for (s <- matches) {
      assert(p.matches(s))
    }
  ...
  def testSplit(regexp: String, text: String, limit: Int, expected: Array[String]): Unit = {
  ...
  def testReplaceAll(orig: String, regex: String, repl: String, actual: String): Unit = {
...
//    assert(actual == replaced)
  ...
  def testReplaceFirst(orig: String, regex: String, repl: String, actual: String): Unit = {
    val p = Pattern.compile(regex)
    val m = p.matcher(orig)
...
//    assert(count == mj.groupCount)
  ...
  def testFind(text: String, regexp: String, start: Int, output: String): Unit = {
    val p = Pattern.compile(regexp)
    val matchString = p.matcher(text)
  ...
  def verifyLookingAt(text: String, regexp: String, output: Boolean): Unit = {
    ...
    assert(output == java.util.regex.Pattern.compile(regexp).matcher(text).lookingAt)
  }
}
